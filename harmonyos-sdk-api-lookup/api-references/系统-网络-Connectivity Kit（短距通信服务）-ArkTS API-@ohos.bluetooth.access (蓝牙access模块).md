...
* 调用该接口时，系统弹出开启蓝牙的对话框，由用户确认是否需要开启蓝牙。如果应用想要感知用户操作对话框的行为，建议使用[access.enableBluetoothAsync](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-access#accessenablebluetoothasync20)。
* 蓝牙开关状态结果可通过[access.on('stateChange')](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-access#accessonstatechange)的回调函数获取到。
* 建议蓝牙开关状态是[STATE\_OFF](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-access#bluetoothstate)时，才调用该接口开启蓝牙（可使用[access.getState](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-access#accessgetstate)判断当前蓝牙开关状态）。
...
* 调用该接口时，系统弹出关闭蓝牙的对话框，由用户确认是否需要关闭蓝牙。如果应用想要感知用户操作对话框的行为，建议使用[access.disableBluetoothAsync](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-access#accessdisablebluetoothasync20)。
* 蓝牙开关状态结果可通过[access.on('stateChange')](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-access#accessonstatechange)的回调函数获取到。
* 建议蓝牙开关状态是[STATE\_ON](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-access#bluetoothstate)时，才调用该接口关闭蓝牙（可使用[access.getState](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-access#accessgetstate)判断当前蓝牙开关状态）。
...
| 类型 | 说明 |
| --- | --- |
| [BluetoothState](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-access#bluetoothstate) | 表示蓝牙开关状态。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| type | string | 是 | 事件回调类型，支持的事件为'stateChange'，表示蓝牙开关状态变化事件。  如：当调用[access.enableBluetooth](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-access#accessenablebluetooth)或[access.disableBluetooth](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-access#accessdisablebluetooth)时，可触发该事件。 |
| callback | Callback<[BluetoothState](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-access#bluetoothstate)> | 是 | 指定订阅的回调函数，会携带蓝牙开关状态。 |
...
| callback | Callback<[BluetoothState](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-access#bluetoothstate)> | 否 | 指定取消订阅的回调函数通知。  若传参，则需与[access.on('stateChange')](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-access#accessonstatechange)中的回调函数一致；若无传参，则取消订阅该type对应的所有回调函数通知。 |
...
* 指定持久化存储的虚拟MAC地址需是有效的（可使用[access.isValidRandomDeviceId](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-access#accessisvalidrandomdeviceid16)判断）。
* 使用该接口时，开发者应确保该虚拟MAC地址对应的对端蓝牙设备实际地址是保持不变的，若对端设备实际地址发生变化，持久化存储的地址信息将失效，无法继续使用。
* 可调用[access.deletePersistentDeviceId](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-access#accessdeletepersistentdeviceid16)删除已持久化存储的虚拟MAC地址。
...
* 该虚拟MAC地址通过[access.addPersistentDeviceId](/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-access#accessaddpersistentdeviceid16)持久化存储。
