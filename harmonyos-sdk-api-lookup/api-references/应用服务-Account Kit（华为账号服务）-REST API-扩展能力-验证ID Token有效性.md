...
| 参数 | 是否必选 | 参数类型 | 描述 |
| --- | --- | --- | --- |
| Content-Type | 是 | String | 取值为：application/x-www-form-urlencoded。  **说明：**  Request Body传参，请务必遵循此请求头格式，否则可能导致请求失败，具体传参方式可参考[示例代码](/consumer/cn/doc/harmonyos-references/account-api-verify-id-token#示例代码)。 |
...
| 参数 | 参数类型 | 描述 |
| --- | --- | --- |
| error | int | 业务响应主错误码，详见[错误码](/consumer/cn/doc/harmonyos-references/account-api-verify-id-token#错误码)。 |
| sub\_error | int | 业务响应子错误码，详见[错误码](/consumer/cn/doc/harmonyos-references/account-api-verify-id-token#错误码)。 |
...
| **参数** | 是否必选 | 参数类型 | 描述 |
| --- | --- | --- | --- |
| anonymized\_login\_mobile\_number | 否 | String | 匿名化的华为账号绑定手机号码，详见[华为账号一键登录（获取手机号和UnionID/OpenID）](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-phone-unionid-login)。 |
