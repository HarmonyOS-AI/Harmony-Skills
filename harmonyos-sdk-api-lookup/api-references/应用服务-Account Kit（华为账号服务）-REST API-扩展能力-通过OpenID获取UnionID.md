...
| 参数 | 是否必选 | 参数类型 | 描述 |
| --- | --- | --- | --- |
| Content-Type | 是 | String | 取值为：application/x-www-form-urlencoded。  **说明：**  Request Body传参，请务必遵循此请求头格式，否则可能导致请求失败，具体传参方式可参考[示例代码](/consumer/cn/doc/harmonyos-references/account-api-get-unionid#示例代码)。 |
...
| 参数 | 是否必选 | 参数类型 | 描述 |
| --- | --- | --- | --- |
| access\_token | 是 | String | 通过[获取应用级凭证](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-obtain-app-token)获取的Access Token。  **说明：**  通过OpenID获取UnionID时，Access Token必须由OpenID所属应用的Client ID获取。 |
...
| 参数 | 参数位置 | 参数类型 | 描述 |
| --- | --- | --- | --- |
| NSP\_STATUS | header | String | 错误码，详见本章节[错误码](/consumer/cn/doc/harmonyos-references/account-api-get-unionid#错误码)。 |
...
| 102 | 无效的access\_token。 | access\_token参数无效，可能原因：请求头的Content-Type为application/x-www-form-urlencoded，但实际代码调用时，未对请求body参数进行URLEncode处理，可参考[示例代码](/consumer/cn/doc/harmonyos-references/account-api-get-unionid#示例代码)组装参数。 |
