本模块提供应用打点和事件订阅能力，包括事件存储、事件订阅、事件清理、打点配置等功能。HiAppEvent将应用运行过程中触发的事件信息统一归纳到[AppEventInfo](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#appeventinfo)中，并将事件分为系统事件和应用事件两类。
...
应用事件来源于应用，是应用开发者自己定义的事件，这类事件信息支持自定义后通过[Write](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#hiappeventwrite-1)打点接口进行配置设定，具体字段含义可结合开发者需求展开。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| watcher | [Watcher](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#watcher) | 是 | 事件观察者。 |
...
| 类型 | 说明 |
| --- | --- |
| [AppEventPackageHolder](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#appeventpackageholder) | 订阅数据持有者。订阅失败时返回null。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| params | Record<string, [ParamType](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#paramtype12)> | 是 | 事件自定义参数对象。参数名和参数值规格定义如下：  - 参数名为string类型，首字符必须为字母字符或$字符。中间字符必须为数字字符、字母字符或下划线字符。结尾字符必须为数字字符或字母字符。长度非空且不超过32个字符。  - 参数值为[ParamType](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#paramtype12)类型，参数值长度需在1024个字符以内。  - 参数个数需在64个以内。 |
| domain | string | 是 | 事件领域。事件领域可支持关联应用事件和系统事件（hiAppEvent.domain.OS）。 |
| name | string | 否 | 事件名称。默认为空字符串，空字符串表示关联事件领域下的所有事件名称。事件名称可支持关联应用事件和系统事件，其中系统事件仅支持关联：  - [崩溃事件](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/hiappevent-watcher-crash-events)（hiAppEvent.event.APP\_CRASH）  - [应用冻屏事件](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/hiappevent-watcher-freeze-events)（hiAppEvent.event.APP\_FREEZE）  - [资源泄漏事件](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/hiappevent-watcher-resourceleak-events)（hiAppEvent.event.RESOURCE\_OVERLIMIT）。  **注意**：从API version 20开始，支持[资源泄漏事件](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/hiappevent-watcher-resourceleak-events)。 |
...
* APP\_CRASH（参数配置详见[崩溃日志配置参数设置介绍](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/hiappevent-watcher-crash-events#崩溃日志规格自定义参数设置)）
* RESOURCE\_OVERLIMIT（参数配置详见[资源泄漏事件检测](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/hiappevent-watcher-resourceleak-events#自定义规格设置)）
...
| --- | --- | --- | --- |
| name | string | 是 | 事件名称。 |
| config | Record<string, [ParamType](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#paramtype12)> | 是 | 事件自定义参数对象。参数名和参数值规格定义如下：  - 参数名为string类型，要求非空，且参数名长度需在1024个字符以内。  - 参数值为ParamType类型，参数值长度需在1024个字符以内。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| policy | [EventPolicy](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#eventpolicy22) | 是 | 系统事件配置策略。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<void> | Promise对象，无返回结果。  各个事件的事件配置策略，详细规格见[EventPolicy](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#eventpolicy22)类型说明。若配置策略设置有误，会导致接口返回失败。  - 参数类型设置有误，则返回401通用错误信息；  - 参数规格设置有误，则在hilog日志输出相关错误信息。 |
...
| triggerCondition | [TriggerCondition](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#triggercondition) | 否 | 是 | 订阅回调触发条件，需要与回调函数onTrigger一同传入才会生效。默认不触发。 |
| appEventFilters | [AppEventFilter](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#appeventfilter)[] | 否 | 是 | 订阅过滤条件，在需要对订阅事件进行过滤时传入。默认不过滤事件。 |
| onTrigger | (curRow: number, curSize: number, holder: [AppEventPackageHolder](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#appeventpackageholder)) => void | 否 | 是 | 订阅回调函数，需要与回调触发条件triggerCondition一同传入才会生效，函数入参说明如下：  curRow：在本次回调触发时的订阅事件总数量；  curSize：在本次回调触发时的订阅事件总大小，单位为byte；  holder：订阅数据持有者对象，可以通过其对订阅事件进行处理。 |
| onReceive11+ | (domain: string, appEventGroups: Array<[AppEventGroup](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#appeventgroup11)>) => void | 否 | 是 | 订阅实时回调函数，与回调函数onTrigger同时存在时，只触发此回调，函数入参说明如下：  domain：回调事件的领域名称；  appEventGroups：回调事件集合。 |

说明

不建议在回调函数中执行[移除观察者](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#hiappeventremovewatcher)的操作，watcher一旦被移除，则其原有的订阅回调功能也会随之失效，可能会造成某些事件发生后无订阅回调情况。

## TriggerCondition

PhonePC/2in1TabletTVWearable

提供设置[Watcher](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#watcher)的onTrigger回调触发条件的参数选项。
...
提供设置[Watcher](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#watcher)的订阅过滤条件的参数选项。用于在事件观察者中设置事件过滤条件，确保只有满足过滤条件的事件才会被监听处理。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| domain | string | 否 | 否 | 需要订阅的事件领域。可以是系统事件领域（hiAppEvent.domain.OS）或开发者在使用[Write](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#hiappeventwrite-1)接口时传入的自定义事件信息（[AppEventInfo](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#appeventinfo)）中的事件领域。 |
| eventTypes | [EventType](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#eventtype)[] | 否 | 是 | 需要订阅的事件类型集合。默认不进行过滤。 |
...
类构造函数，用于创建订阅数据持有者实例。先通过[addWatcher](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#hiappeventaddwatcher)添加事件观察者，再通过观察者名称关联到应用内已添加的观察者对象。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| watcherName | string | 是 | 已通过[addWatcher](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#hiappeventaddwatcher)添加的事件观察者名称。若未通过addWatcher添加，则默认无数据。 |
...
| 类型 | 说明 |
| --- | --- |
| [AppEventPackage](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#appeventpackage) | 取出的事件包对象，订阅事件数据被全部取出后会返回null。 |
...
| appEventInfos12+ | Array<[AppEventInfo](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#appeventinfo)> | 否 | 否 | 事件对象集合。  **元服务API：** 从API version 12开始，该参数支持在元服务中使用。 |

## AppEventGroup11+

PhonePC/2in1TabletTVWearable

提供订阅返回的事件组的参数定义。可用于获取事件组的详细信息，事件组常在[Watcher](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#watcher)的onReceive回调中使用。
...
| appEventInfos | Array<[AppEventInfo](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#appeventinfo)> | 否 | 否 | 事件对象集合。 |
...
应用事件打点方法，将AppEventInfo类型的事件进行存储，使用callback方式作为异步回调。通过此接口写入的事件对象是开发者自定义的对象，为了避免与系统事件产生冲突混淆，不建议写入系统事件（[Event](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#hiappeventevent)中定义的系统事件名称常量）。此接口写入的事件可通过订阅事件观察者（[addWatcher](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#hiappeventaddwatcher)）进行订阅。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| info | [AppEventInfo](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#appeventinfo) | 是 | 应用事件对象。其内部定义的事件名称建议避免与[Event](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#hiappeventevent)中定义的系统事件名称常量产生冲突。 |
...
应用事件打点方法，将AppEventInfo类型的事件进行存储，使用Promise方式作为异步回调。通过此接口写入的事件对象是开发者自定义的对象，为了避免与系统事件产生冲突混淆，不建议写入系统事件（[Event](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#hiappeventevent)中定义的系统事件名称常量）。此接口写入的事件可通过订阅事件观察者（[addWatcher](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#hiappeventaddwatcher)）进行处理。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| info | [AppEventInfo](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#appeventinfo) | 是 | 应用事件对象。其中的事件名称建议避免与[Event](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#hiappeventevent)中定义的系统事件名称常量冲突混淆。 |
...
该接口为同步接口，包含耗时操作。为了确保性能，建议使用[addProcessorFromConfig](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#hiappeventaddprocessorfromconfig20)异步接口或者交由子线程执行。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| processor | [Processor](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#processor11) | 是 | 上报事件的数据处理者。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| id | number | 是 | 上报事件数据处理者ID。值大于0。由调用[addProcessor](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#hiappeventaddprocessor11)或[addProcessorFromConfig](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#hiappeventaddprocessorfromconfig20)接口返回值所得。 |
...
设置用户ID值。用于在配置[Processor](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#processor11)数据处理者时进行关联。
...
设置用户属性值。用于在配置[Processor](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#processor11)数据处理者时进行关联。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| config | [ConfigOption](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#configoption) | 是 | 应用事件打点配置项对象。 |
...
提供系统事件配置策略的定义，用于使用[configEventPolicy](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#hiappeventconfigeventpolicy22)设置事件配置策略。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| mainThreadJankPolicy | [MainThreadJankPolicy](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#mainthreadjankpolicy22) | 否 | 是 | 主线程超时事件配置策略。  **元服务API：** 从API version 22开始，该接口支持在元服务中使用。 |
| cpuUsageHighPolicy | [CpuUsageHighPolicy](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#cpuusagehighpolicy22) | 否 | 是 | CPU高负载事件配置策略。  **元服务API：** 从API version 22开始，该接口支持在元服务中使用。 |
| appCrashPolicy24+ | [AppCrashPolicy](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#appcrashpolicy24) | 否 | 是 | 崩溃事件配置策略。  **元服务API：** 从API version 24开始，该接口支持在元服务中使用。 |
| appFreezePolicy24+ | [AppFreezePolicy](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#appfreezepolicy24) | 否 | 是 | 应用冻屏事件配置策略。  **元服务API：** 从API version 24开始，该接口支持在元服务中使用。 |
| resourceOverlimitPolicy24+ | [ResourceOverlimitPolicy](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#resourceoverlimitpolicy24) | 否 | 是 | 资源泄漏事件配置策略。  **元服务API：** 从API version 24开始，该接口支持在元服务中使用。 |
| addressSanitizerPolicy24+ | [AddressSanitizerPolicy](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#addresssanitizerpolicy24) | 否 | 是 | 地址越界事件配置策略。  **元服务API：** 从API version 24开始，该接口支持在元服务中使用。 |
...
| userIds | string[] | 否 | 是 | 数据处理者可以上报的用户ID的name数组。name对应[setUserId](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#hiappeventsetuserid11)接口的name参数。默认为空数组。  **元服务API：** 从API version 11开始，该参数支持在元服务中使用。 |
| userProperties | string[] | 否 | 是 | 数据处理者可以上报的用户属性的name数组。name对应[setUserProperty](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#hiappeventsetuserproperty11)接口的name参数。默认为空数组。  **元服务API：** 从API version 11开始，该参数支持在元服务中使用。 |
| eventConfigs | [AppEventReportConfig](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#appeventreportconfig11)[] | 否 | 是 | 数据处理者可以上报的事件描述配置数组。默认为空数组。  **元服务API：** 从API version 11开始，该参数支持在元服务中使用。 |
...
提供事件名称常量。包含系统事件名称常量和应用事件名称常量，其中应用事件名称常量是为开发者在调用[Write](/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#hiappeventwrite-1)接口进行应用事件打点时预留的可选自定义事件名称。
