...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| config | [RetrievalConfig](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#retrievalconfig) | 是 | 检索器的配置信息。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<[Retriever](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#retriever)> | Promise对象，返回检索器对象。 |
...
| 名称 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| channelConfigs | Array<[ChannelConfig](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#channelconfig)> | 是 | 不同检索回路的配置信息数组。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| channelType | [ChannelType](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#channeltype) | 否 | 否 | 当前检索回路的数据库类型。 |
...
| dbConfig | [DbConfig](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#dbconfig) | 否 | 否 | 当前检索回路的数据库配置。 |
...
下列接口都需先使用[retrieval.getRetriever](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#getretriever)获取到检索器实例，再通过此实例调用对应接口。
...
| condition | [RetrievalCondition](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#retrievalcondition) | 是 | 检索条件。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<[RdbRecords](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#rdbrecords)> | Promise对象，返回检索召回数据。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| recallConditions | Array<[RecallCondition](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#recallcondition)> | 否 | 否 | 召回的条件，数组中的每个元素对应一个召回操作。 |
| rerankMethod | [RerankMethod](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#rerankmethod) | 否 | 是 | 重排方法。其参数rerankType默认值为RRF算法，参数parameters默认值遵循[RecallCondition](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#recallcondition)中相应检索回路的参数。 |
| resultCount | number | 否 | 是 | 重排后允许返回结果的最大数量。默认值为500。必须为正整数。 |
| explanation | [ExplanationConfig](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#explanationconfig) | 否 | 是 | 检索解释 |
...
| 类型 | 说明 |
| --- | --- |
| [InvertedIndexRecallCondition](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#invertedindexrecallcondition) | 倒排召回条件。 |
| [VectorRecallCondition](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#vectorrecallcondition) | 向量召回条件。 |
...
| primaryKey | Array<[ColumnName](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#columnname)> | 否 | 否 | 召回结果主键字段，会作为召回字段之一，作为多路召回文档聚合依据。一次查询中所有召回操作的主键字段数量之和必须和相等且不能为空。 |
| responseColumns | Array<[ColumnName](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#columnname)> | 否 | 否 | 需要额外召回的字段集合。Array<[ColumnName](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#columnname)>中[ColumnName](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#columnname)不得为空。 |
| invertedIndexStrategies | Array<[InvertedIndexStrategy](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#invertedindexstrategy)> | 否 | 是 | 召回策略列表，决定了倒排表应当如何打分。如果为空，则默认执行全表匹配。 |
| recallName | [RecallName](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#recallname) | 否 | 是 | 当前检索回路的名称，作为重排阶段识别依据。默认值为随机字符串。构造方法中可以不构建该参数，如果有这个参数则值不能为空字符串。 |
| filters | Array<[FilterInfo](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#filterinfo)> | 否 | 是 | 附加的过滤条件。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| columns | Array<[ColumnName](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#columnname)> | 否 | 否 | 被过滤的列名称。 |
| operator | [Operator](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#operator) | 否 | 是 | 过滤条件中的操作算子。operator和filterValue、operator和filterRange至少有一组同时设置，过滤功能才能生效。 |
| filterValue | [FilterValue](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#filtervalue) | 否 | 是 | 过滤条件中的过滤值。 |
| filterRange | [FilterRange](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#filterrange) | 否 | 是 | 过滤条件中的过滤范围。 |
...
| 类型 | 说明 |
| --- | --- |
| [Bm25Strategy](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#bm25strategy) | 部分命中策略，检索目标只要命中查询词中的80%核心词就可以被召回，召回结果基于bm25算法进行评分，适用于对检索目标匹配要求较宽松的场景。 |
| [ExactMatchingStrategy](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#exactmatchingstrategy) | 精确命中策略，要求检索目标和查询词完全匹配才能被召回，适用于对检索目标匹配要求较严格的场景。 |
| [ProximityStrategy](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#proximitystrategy) | 乱序命中策略，查询词的分词结果在检索目标中不必严格按照原本顺序出现，各个分词结果在指定间隔内都出现即可召回，适用于对检索目标匹配要求中等的场景。 |
...
| columnWeight | Record<[ColumnName](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#columnname), number> | 否 | 是 | 指定该策略运用于倒排表的哪些字段，以及这些字段对应的权重（用于控制字段匹配情况对最终结果的影响程度）  如果字段为空，则默认该策略运用于倒排表的全部字段，会默认增加1个配置，字段名为倒排表名，权重值为1.0。  如果不为空，则字段名不为空字符串，权重值为非负数。 |
...
| columnWeight | Record<[ColumnName](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#columnname), number> | 否 | 是 | 指定该策略运用于倒排表的哪些字段，以及这些字段对应的权重（用于控制字段匹配情况对最终结果的影响程度）  如果字段为空，则默认该策略运用于倒排表的全部字段，会默认增加1个配置，字段名为倒排表名，权重值为1.0。  如果不为空，则字段名不为空字符串，权重值为非负数。 |
| columnSlops | Record<[ColumnName](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#columnname), number> | 否 | 是 | 每个字段使用的偏移量配置。默认值和columnWeight相同的字段，值为10。字段不能为空，对应值必须为非负数。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| vectorQuery | [VectorQuery](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#vectorquery) | 否 | 否 | 用于向量检索的查询词向量。 |
...
| primaryKey | Array<[ColumnName](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#columnname)> | 否 | 否 | 召回结果主键字段，会作为召回字段之一，作为多路召回文档聚合依据。一次查询中所有RecallCondition的主键字段数量必须相等且不能为空。 |
| responseColumns | Array<[ColumnName](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#columnname)> | 否 | 否 | 需要额外召回的字段集合。ColumnName不得为空。 |
| recallName | [RecallName](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#recallname) | 否 | 是 | 当前检索回路的名称，作为重排阶段识别依据。默认值为随机字符串，不能为空字符串 |
| filters | Array<[FilterInfo](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#filterinfo)> | 否 | 是 | 额外的过滤条件。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| column | [ColumnName](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#columnname) | 否 | 否 | 待匹配向量字段名。必须为指定的向量数据库中存在的向量类型字段。 |
| value | Float32Array | 否 | 是 | 向量列的向量值。  如果未定义value字段，系统将尝试将原始query生成向量。目前在PC上支持自动生成。  从HarmonyOS 6.0.0 Beta2版本开始，此参数由“必填”变更为“可选”。 |
| similarityThreshold | number | 否 | 是 | 向量阈值，用于过滤不相似向量的阈值。默认值为1，取值范围最小值为0，最大值为[VectorRerankParameter](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#vectorrerankparameter)中有效thresholds的最小值。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| rerankType | [RerankType](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#reranktype) | 否 | 否 | 重排算法，取值类型为枚举类[RerankType](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#reranktype)，可选项为RRF、FUSED\_SCORE，分别指向rrf排序算法以及分数融合排序算法。默认值为RRF。 |
| parameters | Record<[ChannelType](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#channeltype), [RerankParameter](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#rerankparameter)> | 否 | 是 | 每个召回回路对应的重排参数。如果上述检索参数RetrievalCondition中的存在的channelType未在parameters中配置重排参数，则会自动填充该channelType对应的重排参数默认值。倒排的重排参数默认值参考[InvertedIndexRerankParameter](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#invertedindexrerankparameter)，向量的重排参数默认值参考[VectorRerankParameter](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#vectorrerankparameter)。 |
...
| 类型 | 说明 |
| --- | --- |
| [InvertedIndexRerankParameter](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#invertedindexrerankparameter) | 重排参数为InvertedIndexRerankParameter。 |
| [VectorRerankParameter](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#vectorrerankparameter) | 重排参数为VectorRerankParameter。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| invertedIndexWeights | Record<[RecallName](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#recallname), number> | 否 | 否 | 每路召回的重排权重，体现各路召回在重排过程中的重要性。key为各路召回RecallCondition的[RecallName](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#recallname)，value为对应那一路召回的权重。  权重值必须为非负数。如果未定义某一路召回的权重，则默认值为1。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| vectorWeights | Record<[RecallName](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#recallname), number> | 否 | 否 | 每路召回的重排权重，体现各路召回在重排过程中的重要性。key为各路召回RecallCondition的[RecallName](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#recallname)，value为对应那一路召回的权重。权重值必须为非负数。如果未定义某一路召回的权重，则默认值为1。 |
...
| numberInspector | Record<[RecallName](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#recallname), [ColumnName](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#columnname)> | 否 | 是 | 向量召回数字匹配降档策略配置，如果查询中存在数字，但是对应目标字段中的数字和查询数字不匹配，则将该检索结果的相关性降低。key为对应向量召回条件中的[RecallName](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#recallname)，value为对应模式匹配字段名。默认值为空。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| records | Array<[ItemInfo](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#iteminfo)> | 否 | 否 | 检索结果 |
| missedGroundTruths | Array<[ItemInfo](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#iteminfo)> | 否 | 是 | 未检索到指定文档的信息 |
...
| recallScores | Record<[ChannelType](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#channeltype), Record<string, [RecallScore](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#recallscore)>> | 否 | 否 | 每路召回的得分，其反映了每路召回对该结果的相关性评估。key为该路RecallCondition的scoreKey；  - 如果这里没有体现RetrievalCondition中的某一路RecallCondition的scoreKey，则代表该路召回认为该结果相关性过低不予召回。 |
...
| similarityLevel | [SimilarityLevel](/consumer/cn/doc/harmonyos-references/dataaugmentation-retrieval-api#similaritylevel) | 否 | 否 | 根据语义向量距离以及文本匹配程度，对检索结果按照相关性分档，方便对结果进行进一步筛选并且过滤。 |
