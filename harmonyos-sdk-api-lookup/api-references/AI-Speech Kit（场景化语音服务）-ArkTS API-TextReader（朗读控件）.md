...
调用朗读控件接口前，必须先调用[init](/consumer/cn/doc/harmonyos-references/speech-textreader-api#init)初始化，否则会报错。
...
| readParams | [ReaderParam](/consumer/cn/doc/harmonyos-references/speech-textreader-api#readerparam) | 是 | 朗读参数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| readInfoList | [ReadInfo](/consumer/cn/doc/harmonyos-references/speech-textreader-api#readinfo)[] | 是 | 播放文章列表。  - 若列表的第一篇文章是有分类的（即带有category或categoryObject字段），后续文章不带分类，也会归为“其他”分类下。  - 若列表的第一篇是没有分类的，后续文章即使带分类，也不会生效。 |
...
| startParams | [StartParams](/consumer/cn/doc/harmonyos-references/speech-textreader-api#startparams) | 是 | 朗读控件起播参数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| paramName | [ResetParamType](/consumer/cn/doc/harmonyos-references/speech-textreader-api#resetparamtype) | 是 | 重置参数类型。 |
...
| 类型 | 说明 |
| --- | --- |
| [ReadState](/consumer/cn/doc/harmonyos-references/speech-textreader-api#readstate) | 文章播报状态 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| readInfo | [ReadInfo](/consumer/cn/doc/harmonyos-references/speech-textreader-api#readinfo) | 是 | 文章信息，仅更新image、imageUrl、bodyInfo、bodyInfoObject；如果文章的ID不在列表中，则不会生效。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| readInfos | [ReadInfo](/consumer/cn/doc/harmonyos-references/speech-textreader-api#readinfo)[] | 是 | 要加载的文章列表。 |
...
| callback | Callback<[ReadState](/consumer/cn/doc/harmonyos-references/speech-textreader-api#readstate)> | 是 | 正在播放的文章状态变更时，执行的回调函数。 |
...
| --- | --- | --- | --- |
| type | string | 是 | 事件回调类型，支持的事件为'stateChange'，当前正在播放的文章状态变更时，触发该事件。 |
| callback | Callback<[ReadState](/consumer/cn/doc/harmonyos-references/speech-textreader-api#readstate)> | 否 | 需要取消监听的回调函数，需与订阅时传入的回调函数是同一个，若不填，则取消当前应用监听该事件的所有回调函数。 |
...
| callback | Callback<string> | 是 | 拉到播放列表底端或播放到文章最后一篇，触发该回调执行。返回的自定义参数在[StartParams](/consumer/cn/doc/harmonyos-references/speech-textreader-api#startparams)的callbackParam属性配置。 |
...
| callback | Callback<[NotificationEvent](/consumer/cn/doc/harmonyos-references/speech-textreader-api#notificationevent)> | 是 | 播控中心状态发生变化时，触发该回调执行。 |
...
| callback | Callback<[NotificationEvent](/consumer/cn/doc/harmonyos-references/speech-textreader-api#notificationevent)> | 否 | 需要取消监听的回调函数，需与订阅时传入的回调函数是同一个，若不填，则取消当前应用监听该事件的所有回调函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| type | string | 是 | 事件回调类型，支持的事件为'eventPanel'，播放面板状态变化，其中[PanelEvent](/consumer/cn/doc/harmonyos-references/speech-textreader-api#panelevent)中的BPC\_10 click事件回调只支持手机。 |
| callback | Callback<[PanelEvent](/consumer/cn/doc/harmonyos-references/speech-textreader-api#panelevent)> | 是 | 播放面板状态发生变化时，触发该回调执行。 |
...
| callback | Callback<[PanelEvent](/consumer/cn/doc/harmonyos-references/speech-textreader-api#panelevent)> | 否 | 需要取消监听的回调函数，需与订阅时传入的回调函数是同一个，若不填，则取消当前应用监听该事件的所有回调函数。 |
...
| callback | Callback<Array<[ListEventState](/consumer/cn/doc/harmonyos-references/speech-textreader-api#listeventstate)>> | 是 | 播报列表发生相关事件时，触发该回调执行。 |
...
| --- | --- | --- | --- |
| type | string | 是 | 事件回调类型，支持的事件为'eventReadList'。 |
| callback | Callback<Array<[ListEventState](/consumer/cn/doc/harmonyos-references/speech-textreader-api#listeventstate)>> | 否 | 需要取消监听的回调函数，需与订阅时传入的回调函数是同一个，若不填，则取消当前应用监听该事件的所有回调函数。 |
...
| callback | Callback<[ReadProgress](/consumer/cn/doc/harmonyos-references/speech-textreader-api#readprogress)> | 是 | 已播放时长发生相关事件时，触发该回调执行。 |
...
| --- | --- | --- | --- |
| type | string | 是 | 事件回调类型，支持的事件为'readProgress'。 |
| callback | Callback<[ReadProgress](/consumer/cn/doc/harmonyos-references/speech-textreader-api#readprogress)> | 否 | 需要取消监听的回调函数，需与订阅时传入的回调函数是同一个，若不填，则取消当前应用监听该事件的所有回调函数。 |
...
| businessBrandInfo | [BusinessBrandInfo](/consumer/cn/doc/harmonyos-references/speech-textreader-api#businessbrandinfo) | 否 | 是 | 播报品牌信息。  - 默认是：小艺朗读。 |
...
| person | [Person](/consumer/cn/doc/harmonyos-references/speech-textreader-api#person) | 否 | 是 | 朗读人员语气和风格信息，默认参数如下。  { tone: 0, style: 'interaction-broadcast' }  **起始版本：** 5.0.2(14) |
...
| minibarParams | [MinibarParams](/consumer/cn/doc/harmonyos-references/speech-textreader-api#minibarparams) | 否 | 是 | Minibar位置设置，默认吸附左边。  **起始版本：** 5.0.2(14) |
| customFeatures | [CustomFeature[]](/consumer/cn/doc/harmonyos-references/speech-textreader-api#customfeature) | 否 | 是 | 朗读控件的自定义功能，默认空。  **起始版本：** 5.0.2(14) |
| displayTab | [DisplayTab](/consumer/cn/doc/harmonyos-references/speech-textreader-api#displaytab) | 否 | 是 | 显示Tab页配置，默认显示封面和内容页。  **起始版本：** 5.0.2(14) |
...
| 名称 | **类型** | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| defaultAlignment | [BarAlignment](/consumer/cn/doc/harmonyos-references/speech-textreader-api#baralignment) | 否 | 否 | Minibar默认吸附位置。  **元服务API：** 从版本5.0.3(15)开始，该接口支持在元服务中使用。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| type | [CustomFeatureType](/consumer/cn/doc/harmonyos-references/speech-textreader-api#customfeaturetype) | 否 | 否 | 自定义特征的类型，系统应用或者用户自定义功能。 |
...
| title | [TextInfo](/consumer/cn/doc/harmonyos-references/speech-textreader-api#textinfo) | 否 | 否 | 标题。 |
| category | string | 否 | 是 | 类目，默认空。 |
| author | [TextInfo](/consumer/cn/doc/harmonyos-references/speech-textreader-api#textinfo) | 否 | 是 | 文章的作者，默认空。 |
| date | [TextInfo](/consumer/cn/doc/harmonyos-references/speech-textreader-api#textinfo) | 否 | 是 | 文章的时间（例:Mon Jul 08 2024 21:25:41 GMT+0800）。  预留参数，暂未支持。 |
...
| bodyInfo | string | 否 | 是 | [setArticle](/consumer/cn/doc/harmonyos-references/speech-textreader-api#setarticle)以及[loadMore](/consumer/cn/doc/harmonyos-references/speech-textreader-api#loadmore)接口必填。  实时朗读的正文信息。  从6.1.0(23)版本开始，正文长度支持10000字符以上。6.0.2(22)及之前版本，正文中无标点符号和换行符的情况下，长度须小于等于10000字符。 |
| bodyInfoObject | [BodyInfo](/consumer/cn/doc/harmonyos-references/speech-textreader-api#bodyinfo) | 否 | 是 | 正文内容信息，默认空。  **起始版本：** 5.0.2(14) |
| categoryObject | [CategoryInfo](/consumer/cn/doc/harmonyos-references/speech-textreader-api#categoryinfo) | 否 | 是 | 文章分类信息，默认空。  **起始版本：** 5.0.2(14) |
| audioInfo | [AudioInfo](/consumer/cn/doc/harmonyos-references/speech-textreader-api#audioinfo)[] | 否 | 是 | 音频信息，默认空。  若配置此参数，则优先使用传入音频进行播报。  **起始版本：** 5.0.2(14) |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| type | [BodyInfoType](/consumer/cn/doc/harmonyos-references/speech-textreader-api#bodyinfotype) | 否 | 否 | 类型。 |
...
| person | [Person](/consumer/cn/doc/harmonyos-references/speech-textreader-api#person) | 否 | 是 | 音频信息的声音信息，包括音色、风格，语言等。  默认为如下参数  { tone: 3, style: 'interaction-broadcast' } |
...
| readProgressType | [ReadProgressType](/consumer/cn/doc/harmonyos-references/speech-textreader-api#readprogresstype) | 否 | 否 | 定时上报类型。  **元服务API：** 从版本5.0.3(15)开始，该接口支持在元服务中使用。 |
| person | [Person](/consumer/cn/doc/harmonyos-references/speech-textreader-api#person) | 否 | 否 | 正在播报的文章使用的音色。  **元服务API：** 从版本5.0.3(15)开始，该接口支持在元服务中使用。 |
...
| currentPercent | number | 否 | 否 | 触发该事件回调时，返回用户当前播放的百分比，取值[0,100]。可用于记录上次播放位置，和[start](/consumer/cn/doc/harmonyos-references/speech-textreader-api#start-1)接口配合实现下次继续播放。  **元服务API：** 从版本6.0.1(21)开始，该接口支持在元服务中使用。  **起始版本：** 6.0.1(21) |
