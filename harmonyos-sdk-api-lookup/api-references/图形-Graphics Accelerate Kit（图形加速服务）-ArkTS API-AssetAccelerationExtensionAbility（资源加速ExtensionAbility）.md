...
| **参数名** | **类型** | 必填 | **说明** |
| --- | --- | --- | --- |
| requestType | [ContentRequestType](/consumer/cn/doc/harmonyos-references/graphics-accelerate-extensionability#contentrequesttype) | 是 | 内容请求类型。 |
| manifestUrl | string | 是 | 资源包下载URL：  - 使用华为CDN托管资源，则此参数不为空。  - 使用三方CDN，则此参数为空。  默认值：空字符串。 |
| assetAccelerationExtensionInfo | [AssetAccelerationExtensionInfo](/consumer/cn/doc/harmonyos-references/graphics-accelerate-extensionability#assetaccelerationextensioninfo) | 是 | 资源加速ExtensionAbility信息。 |
