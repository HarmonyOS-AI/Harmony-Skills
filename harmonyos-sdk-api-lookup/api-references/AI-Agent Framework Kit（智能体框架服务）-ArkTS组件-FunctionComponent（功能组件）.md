...
| options | [FunctionOptions](/consumer/cn/doc/harmonyos-references/hmaf-function-component#functionoptions) | 否 | - | Function组件初始化可选参数。 |
| controller | [FunctionController](/consumer/cn/doc/harmonyos-references/hmaf-function-component#functioncontroller) | 否 | - | Function组件控制器。默认为undefined。 |
...
Function组件控制器，用于与Function组件控制交互。预留接口，当前版本完全继承[AgentController](/consumer/cn/doc/harmonyos-references/hmaf-function-component#agentcontroller)的方法，无额外的实现。
...
| 参数名 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| title | string | 否 | 是 | 表示AgentFramework组件的标题。默认值为空。  [FunctionOptions](/consumer/cn/doc/harmonyos-references/hmaf-function-component#functionoptions).buttonType按钮类型为ButtonType.CIRCLE时不体现。  当前只显示指定大小，宽度超过8个中文字符会进行省略。  字体大于1.75倍时默认最大显示两行。 |
...
功能组件选项的定义。继承自[BaseOptions](/consumer/cn/doc/harmonyos-references/hmaf-function-component#baseoptions)。
...
| buttonType | [ButtonType](/consumer/cn/doc/harmonyos-references/hmaf-function-component#buttontype) | 否 | 是 | 功能组件的按钮类型。title属性为空或不传时默认值为ButtonType.CIRCLE，title属性不为空时默认值为ButtonType.CAPSULE。 |
| isShowShadow | boolean | 否 | 是 | 如果显示按钮的阴影，仅在 ButtonType.CAPSULE 胶囊类型时有效。默认值为false。  true：显示阴影。  false：不显示阴影。 |
| backgroundColor | [ResourceColor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-types#resourcecolor) | 否 | 是 | 显示背景板时，组件的背景颜色。  默认值为sys.color.comp\_background\_tertiary背景色。 |
