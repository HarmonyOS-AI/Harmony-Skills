...
用户点击设备后，会触发设备选择控制器对应的接口回调，并返回已选择的设备信息；应用可以通过获取到的设备信息处理分布式业务，比如通过startAbility启动远端UIAbility；待启动远端ability成功后，可以通过[updateState](/consumer/cn/doc/harmonyos-references/servicecollaboration-devicepicker#updatestate)接口更新业务状态，界面上将会显示指定的状态。
...
| 类型 | 说明 |
| --- | --- |
| [DevicePickerController](/consumer/cn/doc/harmonyos-references/servicecollaboration-devicepicker#devicepickercontroller) | 返回设备选择控制器实例，可以与流转控件[CollaborationDevicePicker (流转控件)](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/servicecollaboration-collaborationdevicepicker)配合使用。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| event | [DeviceEvent](/consumer/cn/doc/harmonyos-references/servicecollaboration-devicepicker#deviceevent) | 是 | 设备相关的事件，目前支持：设备选择、设备取消选择、已选择设备的下线事件。 |
...
| state | [BusinessState](/consumer/cn/doc/harmonyos-references/servicecollaboration-devicepicker#businessstate) | 是 | 业务状态，当前支持IDLE、SUCCESSFUL、FAILED三种状态类型。 |
...
释放设备选择控制器，与[createDevicePickerController](/consumer/cn/doc/harmonyos-references/servicecollaboration-devicepicker#createdevicepickercontroller)方法对应。

在应用不需要设备选择控制器的时候需要调用release()接口进行释放，比如在应用onCreate()生命周期回调中通过[createDevicePickerController](/consumer/cn/doc/harmonyos-references/servicecollaboration-devicepicker#createdevicepickercontroller)创建了设备选择控制器，可以在onDestroy()生命周期回调中通过release()进行释放。
