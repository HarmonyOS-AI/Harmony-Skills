...
| **参数** | **是否必填** | **参数类型** | **描述** |
| --- | --- | --- | --- |
| app\_id | 否 | String | 应用ID。获取方式请参见[AppID管理及关联](https://developer.huawei.com/consumer/cn/doc/pay-docs/hwzf-appidguanli-0000001757041165)。  **说明：** 服务商模式接入，切换到商户应用/元服务拉起收银台时，需要把app\_id改成商户相应的appId，并在[平台类商户/服务商预下单](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/payment-agent-prepay)接口通过subAppId字段同步传递。 |
...
| reserved | 否 | String | 扩展字段，jsonStr格式。参见[reserved](/consumer/cn/doc/harmonyos-references/payment-model#reserved)说明。 |
...
| **参数** | **是否必填** | **参数类型** | **描述** |
| --- | --- | --- | --- |
| headers | 是 | [Map](/consumer/cn/doc/harmonyos-references/payment-model#map) | 下载鉴权信息。 |
...
| goodsDetail | 否 | List<[GoodDetail](/consumer/cn/doc/harmonyos-references/payment-model#gooddetail)> | 订单详细信息列表。 |
...
| promotionDetail | 否 | List<[PromotionItem](/consumer/cn/doc/harmonyos-references/payment-model#promotionitem)> | 营销信息。 |
...
| **参数** | **是否必选** | **参数类型** | **描述** |
| --- | --- | --- | --- |
| platCoupons | 否 | List<[PlatCoupon](/consumer/cn/doc/harmonyos-references/payment-model#platcoupon)> | 平台券信息（可参见[查询用户可用券](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/payment-api-common-promotion-service-inquiry)接口获取）。 |
...
| sceneParam | 是 | String | 券因子，jsonStr格式，可参见[sceneParams](/consumer/cn/doc/harmonyos-references/payment-model#sceneparams)。  如平台券有订单金额限制示例：{"tradeOrderAmount": 200} |
...
| [sceneParams](/consumer/cn/doc/harmonyos-references/payment-model#sceneparams) | 是 | Map<String, Object> | 场景扩展参数，用于匹配平台券的使用条件。部分参数可参考[设备信息](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-device-info) |
...
| displayInfo | 否 | [SimpleDisplayInfo](/consumer/cn/doc/harmonyos-references/payment-model#simpledisplayinfo) | 优惠券简易展示信息。 |
...
| **参数** | **是否必填** | **参数类型** | **描述** |
| --- | --- | --- | --- |
| availableTime | 否 | [CouponAvailableTime](/consumer/cn/doc/harmonyos-references/payment-model#couponavailabletime) | 优惠券的日期类核销条件。 |
| fixedNormalCoupon | 否 | [FixedNormalCoupon](/consumer/cn/doc/harmonyos-references/payment-model#fixednormalcoupon) | 固定面额满减券使用规则。  **说明**：当优惠券类型（couponType）为满减券（NORMAL）时，该参数必传。 |
| discountCoupon | 否 | [DiscountCoupon](/consumer/cn/doc/harmonyos-references/payment-model#discountcoupon) | 折扣券使用规则。  **说明**：当优惠券类型（couponType）为折扣券（DISCOUNT）时，该参数必传。 |
| exchangeCoupon | 否 | [ExchangeCoupon](/consumer/cn/doc/harmonyos-references/payment-model#exchangecoupon) | 换购券使用规则。  **说明**：当优惠券类型（couponType）为换购券（EXCHANGE）时，该参数必传。 |
| useMethod | 是 | List<String> | 核销方式。  - FASTAPP：应用/元服务 |
...
| displayInfo | 否 | [CouponDisplayInfo](/consumer/cn/doc/harmonyos-references/payment-model#coupondisplayinfo) | 优惠券展示信息。 |
| couponUseRule | 是 | [CouponUseRule](/consumer/cn/doc/harmonyos-references/payment-model#couponuserule) | 券核销相关规则。 |
...
| couponCodeCount | 否 | [CouponCodeCount](/consumer/cn/doc/harmonyos-references/payment-model#couponcodecount) | 批次券code数量统计。 |
| sendCountInfo | 否 | [SendCountInfo](/consumer/cn/doc/harmonyos-references/payment-model#sendcountinfo) | 批次已发放总量信息。 |
...
| couponUseRule | 是 | [CouponUseRule](/consumer/cn/doc/harmonyos-references/payment-model#couponuserule) | 券使用规则。 |
| couponSendRule | 是 | [CouponSendRule](/consumer/cn/doc/harmonyos-references/payment-model#couponsendrule) | 券发放规则。 |
| displayInfo | 是 | [CouponDisplayInfo](/consumer/cn/doc/harmonyos-references/payment-model#coupondisplayinfo) | 优惠券展示信息。 |
...
| notifyConfig | 否 | [NotifyConfig](/consumer/cn/doc/harmonyos-references/payment-model#notifyconfig) | 通知回调配置。 |
...
| availableWeek | 否 | [AvailableWeek](/consumer/cn/doc/harmonyos-references/payment-model#availableweek) | 固定周期有效时间段，可设置多个星期下的多个可用时间段，比如每周二10点到18点。 |
| irregularyTime | 否 | List<[IrregularyTime](/consumer/cn/doc/harmonyos-references/payment-model#irregularytime)> | 无规律的有效时间段。 |
...
| dayTime | 否 | List<[CurrentDayTime](/consumer/cn/doc/harmonyos-references/payment-model#currentdaytime)> | 当天可用时间段，最多不超过2个，具体定义参考CurrentDayTime结构定义。 |
