...
1. 需要申请ohos.permission.ACCESS\_BLUETOOTH权限。如何配置和申请权限，具体操作请参考[声明权限](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/declare-permissions)和[向用户申请授权](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/request-user-authorization)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| channelInfo | [ChannelInfo](/consumer/cn/doc/harmonyos-references/js-apis-proxychannelmanager#channelinfo) | 是 | 对端设备及服务的MAC和UUID信息。 |
...
| channelId | number | 是 | 打开代理通道时获取的channelId。 |
| callback | Callback<[DataInfo](/consumer/cn/doc/harmonyos-references/js-apis-proxychannelmanager#datainfo)> | 是 | 回调函数，返回接收到的数据。多次注册回调函数，最后一次注册的回调函数生效。 |
...
| type | string | 是 | 设置订阅类型，固定取值为'receiveData'。 |
| channelId | number | 是 | 打开代理通道时获取的channelId。 |
| callback | Callback<[DataInfo](/consumer/cn/doc/harmonyos-references/js-apis-proxychannelmanager#datainfo)> | 否 | 注册的回调函数。如果为空、undefined、null，则取消订阅所有的数据接收事件。如果不为空，传入最后一次注册的回调函数。 |
...
| channelId | number | 是 | 打开代理通道时获取的channelId。 |
| callback | Callback<[ChannelStateInfo](/consumer/cn/doc/harmonyos-references/js-apis-proxychannelmanager#channelstateinfo)> | 是 | 回调函数，返回接收到的通道状态。多次注册callback，最后一次注册的callback生效 |
...
| channelId | number | 是 | 打开代理通道时获取的channelId。 |
| callback | Callback<[ChannelStateInfo](/consumer/cn/doc/harmonyos-references/js-apis-proxychannelmanager#channelstateinfo)> | 否 | 注册的回调函数。如果为空、undefined、null，则取消订阅所有的数据接收事件。如果不为空，传入最后一次注册的回调函数。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| linkType | [LinkType](/consumer/cn/doc/harmonyos-references/js-apis-proxychannelmanager#linktype) | 否 | 否 | 代理通道的链路类型。 |
...
| --- | --- | --- | --- | --- |
| channelId | number | 否 | 否 | 代理通道的channelId。 |
| state | [ChannelState](/consumer/cn/doc/harmonyos-references/js-apis-proxychannelmanager#channelstate) | 否 | 否 | 通道的连接状态。 |
