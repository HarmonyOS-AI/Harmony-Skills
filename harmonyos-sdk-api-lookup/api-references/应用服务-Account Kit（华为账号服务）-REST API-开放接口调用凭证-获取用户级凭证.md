...
| 参数 | 是否必选 | 参数类型 | 描述 |
| --- | --- | --- | --- |
| Content-Type | 是 | String | 取值为：application/x-www-form-urlencoded。  **说明**：  Request Body传参，请务必遵循此请求头格式，否则可能导致请求失败，具体传参方式可参考[示例代码](/consumer/cn/doc/harmonyos-references/account-api-obtain-user-token#示例代码)。 |
...
| 参数 | 参数类型 | 描述 |
| --- | --- | --- |
| error | int | 业务响应主错误码，详见[错误码](/consumer/cn/doc/harmonyos-references/account-api-obtain-user-token#错误码)。 |
| sub\_error | int | 业务响应子错误码，详见[错误码](/consumer/cn/doc/harmonyos-references/account-api-obtain-user-token#错误码)。 |
...
| 1101 | 20152 | code格式不正确。 | 检查code格式是否满足正则：^[0-9a-zA-Z=/\\+]+$。  该错误码出现可能场景：  - code参数被篡改，导致格式不符。  - 请求头的Content-Type为application/x-www-form-urlencoded，但实际代码调用时，未对请求body体进行URLEncode处理，可参考[示例代码](/consumer/cn/doc/harmonyos-references/account-api-obtain-user-token#示例代码)组装参数。 |
