...
如果应用不希望使用HEIF图片，可以借助[Image Kit](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/image-overview)提供的图片编解码能力，自行[将HEIF图片转码为JPEG或PNG格式的图片](/consumer/cn/doc/harmonyos-guides/heif-adapter-faq#担心使用heif格式图片存在兼容性问题需使用jpeg或png格式的图片如何操作)。
...
8. export async function reEncoding(context : Context, fd : number) {
...
20. let file = fs.openSync(filePath, fs.OpenMode.CREATE | fs.OpenMode.READ_WRITE);
21. imagePackerApi.packToFile(imageSource, file.fd, packOpts).then(() => {
22. promptAction.showToast({ message: `Succeed to pack the image.`});
23. console.info('Succeed to pack the image.');
24. }).catch((error : BusinessError) => {
25. promptAction.showToast({ message: 'Failed to pack the image. And the error is: ' + error});
26. console.error('Failed to pack the image. And the error is: ' + error);
27. }).finally(()=>{
28. fs.closeSync(file.fd);
29. })
30. }
```
