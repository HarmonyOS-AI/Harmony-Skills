...
3. import { HiLog } from '../../common/HiLog';
...
12. } catch (err) {
13. HiLog.error(TAG, 'openSync failed. ' + err);
14. if (file) {
...
25. } catch (err) {
26. HiLog.error(TAG,'readSync failed. ' + err);
27. } finally {
28. if (file) {
...
```
1. import fileIo from '@ohos.file.fs';
2. import { HiLog } from '../../common/HiLog';
...
11. HiLog.error(TAG, 'getDLPPermissionInfo: ' + JSON.stringify(err));
12. });
```
...
1. 设置配置信息。
   ...
   10. } catch (err) {
   11. HiLog.error(TAG, 'setSandboxAppConfig error: ' + JSON.stringify(err)); // 失败报错
   12. }
   13. }
   ```
2. 清理配置信息。
   ...
   10. } catch (err) {
   11. HiLog.error(TAG, 'cleanSandboxAppConfig error: ' + JSON.stringify(err)); // 失败报错
   12. }
   13. }
   ```
3. 获取配置信息。
   ...
   10. } catch (err) {
   11. HiLog.error(TAG, 'getSandboxAppConfig error: ' + JSON.stringify(err)); // 失败报错
   12. }
   13. }
   ```
...
* **普通应用未启动，无法感知到DLP沙箱分身打开的DLP文件**。
  ...
  10. HiLog.info(TAG, 'res' + JSON.stringify(res))
  11. } catch (err) {
  12. HiLog.error(TAG, 'error:' + JSON.stringify(err)); // 失败报错
  13. }
  14. }
  ```
* **普通应用已启动，可以感知到DLP沙箱分身打开的DLP文件**。
  ...
  4. import { HiLog } from '../../common/HiLog';
  ...
  19. HiLog.info(TAG, 'openDlpFile event');
  20. }
  ...
  25. } catch (err) {
  26. HiLog.error(TAG, 'error:' + JSON.stringify(err)); // 失败报错
  27. }
  28. }
  ...
  33. } catch (err) {
  34. HiLog.error(TAG, 'error:' + JSON.stringify(err)); // 失败报错
  35. }
  36. }
  37. }
  ```
...
5. import { HiLog } from '../../common/HiLog';
...
22. try {
23. HiLog.info(TAG, 'openDLPFile:' + JSON.stringify(want));
24. HiLog.info(TAG, 'openDLPFile: delegator:' + JSON.stringify(context));
25. context.startAbility(want);
26. } catch (err) {
27. HiLog.error(TAG, 'openDLPFile startAbility failed:' + JSON.stringify(err));
28. return;
29. }
30. }
```
...
* **普通应用内权限设置**
  ...
  20. HiLog.info(TAG, 'startDLPManagerForResult res.resultCode:' + res.resultCode);
  21. HiLog.info(TAG, 'startDLPManagerForResult res.want:' + JSON.stringify(res.want));
  22. }); // 拉起DLP权限管理应用 设置权限
  23. } catch (err) {
  24. HiLog.error(TAG, 'startDLPManagerForResult error:' + (err as BusinessError).code + (err as BusinessError).message);
  25. }
  ```
* **DLP沙箱分身内权限修改，查看和解除**
  ...
  + DLP沙箱分身中可以调用[getDLPPermissionInfo](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-dlppermission#dlppermissiongetdlppermissioninfo)方法查询当前用户DLP文件权限，DLP沙箱分身的权限限制，参考[沙箱限制](/consumer/cn/doc/harmonyos-guides/dlp-guidelines#沙箱限制)。
    ...
    7. dlpPermission.getDLPPermissionInfo().then((data:dlpPermission.DLPPermissionInfo)=> {
    8. HiLog.info(TAG, 'getDLPPermissionInfo, result: ' + JSON.stringify(data));
    9. }).catch((err: BusinessError) => {
    10. HiLog.error(TAG, 'getDLPPermissionInfo: ' + JSON.stringify(err));
    11. });
    ```
...
* **判断一个文件是否是DLP文件**
  ...
  12. HiLog.info(TAG, 'res' +  + JSON.stringify(res));
  13. } catch (err) {
  14. HiLog.error(TAG, 'startDLPManagerForResult error:' + (err as BusinessError).code + (err as BusinessError).message); // 失败报错
  15. }
...
* **判断当前所在应用是否是DLP沙箱分身**
  ...
  8. HiLog.info(TAG, 'isInSandbox, result: ' + JSON.stringify(data));
  9. }).catch((err: BusinessError) => {
  10. HiLog.error(TAG, 'isInSandbox: ' + JSON.stringify(err));
  11. });
  ```
* **保留沙箱**。
  ...
  + 调用接口[setRetentionState](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-dlppermission#dlppermissionsetretentionstate)设置保留沙箱，传入参数为本沙箱内打开的dlp文件的uri列表，该接口只允许在沙箱中调用。
    ...
    12. } catch (err) {
    13. HiLog.error(TAG, 'startDLPManagerForResult error:' + (err as BusinessError).code + (err as BusinessError).message); // 失败报错
    14. }
    15. }
    ```
  ...
  + 调用接口[getRetentionSandboxList](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-dlppermission#dlppermissiongetretentionsandboxlist)获取保留沙箱记录，该接口允许原应用和DLP沙箱分身中调用。
    ...
    10. HiLog.info(TAG, 'res' + JSON.stringify(res))
    11. } catch (err) {
    12. HiLog.error(TAG, 'startDLPManagerForResult error:' + (err as BusinessError).code + (err as BusinessError).message);// 失败报错
    13. }
    14. }
    ```
