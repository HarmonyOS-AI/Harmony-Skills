...
2. 创建自定义响应处理程序，在[HttpEventsHandler](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/remote-communication-rcp#section87603011401)中设置onDataReceive（当接收到HTTP响应正文的一部分时调用的回调）、onHeaderReceive（用于在响应期间处理接收到的headers的回调）、onDataEnd（数据传输完成时触发的回调）。
   ...
   4. // 用于处理传入数据的自定义逻辑
   5. console.info('Received data:', JSON.stringify(incomingData));
...
4. 调用rcp.createSession()传入tracingConfig ，创建通信会话对象session。
   ...
   6. console.error(`err: err code is ${err.code}, err message is ${JSON.stringify(err)}`);
   7. });
   ```
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ff/v3/duONntOFQouKPYh802hrhA/zh-cn_image_0000002282446270.png?HW-CC-KV=V1&HW-CC-Date=20260403T140631Z&HW-CC-Expire=86400&HW-CC-Sign=9259F5A7E47A61D13B0F958122795C6CD430209CA363EF7370418FA738BF70B3 "点击放大")
...
34. console.error(`Response err, the err is ${JSON.stringify(err)}`);
35. })
```