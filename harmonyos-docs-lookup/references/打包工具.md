...
* [Stage模型](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/application-models#应用模型概况)示例：
  ...
  ```
  1. java -jar app_packing_tool.jar --mode hap --json-path <path> [--resources-path <path>] [--ets-path <path>] [--index-path <path>] [--pack-info-path <path>] [--lib-path <path>] --out-path <path> [--force true] [--compress-level 5] [--pkg-context-path <path>] [--hnp-path <path>]
  ```
...
```
1. java -jar app_packing_tool.jar --mode hsp --json-path <path> [--resources-path <path>] [--ets-path <path>] [--index-path <path>] [--pack-info-path <path>] [--lib-path <path>] --out-path <path> [--force true] [--compress-level 5] [--pkg-context-path <path>]
```
...
参考[asanEnabled配置错误码](/consumer/cn/doc/harmonyos-guides/packing-tool#section10012004-检查参数asanenabled失败)、[hwasanEnabled配置错误码](/consumer/cn/doc/harmonyos-guides/packing-tool#section10012005-检查参数hwasanenabled失败)、[atomicService配置错误码](/consumer/cn/doc/harmonyos-guides/packing-tool#section10012006-检查atomicservice失败)、[continueBundleName配置错误码](/consumer/cn/doc/harmonyos-guides/packing-tool#section10012007-检查continuebundlename无效)，更改配置项。
...
1. 存在两个以上的[HSP包](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/in-app-hsp)。例如下图使用DevEco Studio构建App时，工程中包含了两个HSP包library和library1，此时打包APP包失败。
   ...
   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/75/v3/jQRYOxpiSOqfVDXwHB4SCQ/zh-cn_image_0000002535948886.png?HW-CC-KV=V1&HW-CC-Date=20260403T142715Z&HW-CC-Expire=86400&HW-CC-Sign=AF4EF012156FC7782FC329C79AC8770A91F032B72435AE0C8B6EB793C51AB4B2)
...
1. 参考[asanEnabled配置错误码](/consumer/cn/doc/harmonyos-guides/packing-tool#section10012004-检查参数asanenabled失败)、[hwasanEnabled配置错误码](/consumer/cn/doc/harmonyos-guides/packing-tool#section10012005-检查参数hwasanenabled失败)、[atomicService配置错误码](/consumer/cn/doc/harmonyos-guides/packing-tool#section10012006-检查atomicservice失败)、[continueBundleName配置错误码](/consumer/cn/doc/harmonyos-guides/packing-tool#section10012007-检查continuebundlename无效)，更改配置项。
2. 参考[检查overlay失败](/consumer/cn/doc/harmonyos-guides/packing-tool#section10012008-检查overlay失败)，更改配置项。
...
元服务包及依赖的共享库或资源文件大小超出了限制。具体限制请参考[表5 App打包指令参数说明](/consumer/cn/doc/harmonyos-guides/packing-tool#app打包指令)中--atomic-service-entry-size-limit和--atomic-service-non-entry-size-limit参数描述。
...
单个包大小超过限制。具体限制请参考[表5 App打包指令参数说明](/consumer/cn/doc/harmonyos-guides/packing-tool#app打包指令)中--atomic-service-entry-size-limit和--atomic-service-non-entry-size-limit参数描述。
...
单个包和其依赖的共享库总大小超过限制。具体限制请参考[表5 App打包指令参数说明](/consumer/cn/doc/harmonyos-guides/packing-tool#app打包指令)中--atomic-service-entry-size-limit和--atomic-service-non-entry-size-limit参数描述。
